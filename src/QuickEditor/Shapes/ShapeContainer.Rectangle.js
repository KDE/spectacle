...
function ShapeContainer(x, y, w, h, sStyle, fStyle, lWidth, dFunc, hFunc, hsStyle) {
    ...
    this.hsStyle = hsStyle || Qt.rgba(0.188, 0.368, 0.431, 1);

    // the draw function
    this.drawShapeFunc = dFunc || function(ctx, xx, yy, ww, hh, ssStyle, ffStyle, llWidth) {};
    this.drawHandleFunc = hFunc || function(ctx, xx, yy, ww, hh, ssStyle, ffStyle, llWidth) {
        // draw the border first
        ctx.strokeStyle = ssStyle;
        ctx.lineWidth = llWidth;
        ...
        // top middle handle
        ctx.beginPath();
        ctx.arc(xx + (ww / 2), yy, 10, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fill();

        // top right handle
        ctx.beginPath();
        ctx.arc(xx + ww, this.y, 10, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fill();

        // middle left handle
        ctx.beginPath();
        ctx.arc(xx, yy + (hh / 2), 10, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fill();

        // middle right handle
        ctx.beginPath();
        ctx.arc(xx + ww, yy + (hh / 2), 10, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fill();

        // bottom left handle
        ctx.beginPath();
        ctx.arc(xx, yy + hh, 10, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fill();

        // bottom middle handle
        ctx.beginPath();
        ctx.arc(xx + (ww / 2), yy + hh, 10, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fill();

        // bottom right handle
        ctx.beginPath();
        ctx.arc(xx + ww, yy + hh, 10, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fill();
    };
    ...
    this.drawShape = function(ctx) {
        ctx.save();
        this.drawShapeFunc(ctx, this.x, this.y, this.w, this.h, this.sStyle, this.fStyle, this.lWidth);
        ctx.restore();
    }
    ...
    this.drawHandles = function(ctx) {
        ctx.save();
        this.drawHandleFunc(ctx, this.x, this.y, this.w, this.h, this.hsStyle, this.hsStyle, this.lWidth);
        ctx.restore();
    }
