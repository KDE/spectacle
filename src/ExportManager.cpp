...
void ExportManager::exportImage(ExportManager::Actions actions, QUrl url)
{
    ...
    if (actions & SaveAs) {
        ...
        auto dialog = std::make_shared<QFileDialog>();
        ...
        connect(dialog.get(), &QFileDialog::finished, this, [actions, url, dialog](int result) mutable {
...
void ExportManager::exportVideo(ExportManager::Actions actions, const QUrl &inputUrl, QUrl outputUrl)
{
    ...
    if (actions & SaveAs) {
        ...
        // Don't use exec() like the QFileDialog docs show.
        // It can cause a race condition that leads to a crash when the QML environment is being destroyed.
        connect(dialog.get(), &QFileDialog::finished, this, [actions, inputUrl, outputUrl, dialog](int result) mutable {
            const bool accepted = result == QDialog::Accepted;
            const auto &selectedUrl = dialog->selectedUrls().value(0, QUrl());
            if (accepted && !selectedUrl.fileName().isEmpty()) {
